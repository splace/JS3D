...
// change viewangle with wheel on mouse (there are different names and parameters for this event WTF !!!) 
content.addEventListener("mousewheel",function(event){viewAngle*=Math.pow(.9995,-event.wheelDelta);event.preventDefault();drawAll(objects)}, false);
content.addEventListener("DOMMouseScroll",function(event){viewAngle*=Math.pow(.98,event.detail);event.preventDefault();drawAll(objects)}, false);


content.addEventListener("mousedown",function(event){if(event.button==1){responseToMouse.mode=(responseToMouse.mode+1) % responseToMouse.modes.length;modeDisplay.textContent=responseToMouse.modes[responseToMouse.mode].name;}}, false);
...
xl=event.clientX;
yl=event.clientY;
responseToMouse.content.addEventListener("mousemove",responseToMouse, false);
responseToMouse.content.addEventListener("touchmove",responseToMouse, false);
//geometry.setAttribute("fill","LightGrey");
};
};
function removePanListener(event){
if (event.button==0){
responseToMouse.content.removeEventListener("mousemove",responseToMouse, false);
responseToMouse.content.removeEventListener("touchmove",responseToMouse, false);
...
content.addEventListener("touchstart",addPanListener, true);
content.addEventListener("touchend",removePanListener, true);

calcPathSegList=function(pathElement,object,matrix,facingAway){
rdummy=1000/viewAngle;
var ro=rdummy/2;
var loc={x:0,y:0};
var loc0={x:0,y:0};
var xl,yl;
...
var k;
pathElement.pathSegList.clear()
for (var j=0;object.indexes.length>j;j++){
itemIndexes=object.indexes[j];
vec=coords[itemIndexes[0]];
// save some time pre calcularting normals
if (arguments.length>2 && (facingAway?coords[itemIndexes[1]].subtract(vec).cross(coords[itemIndexes[itemIndexes.length-1]].subtract(vec)).dot(vec)<0:coords[itemIndexes[1]].subtract(vec).cross(coords[itemIndexes[itemIndexes.length-1]].subtract(vec)).dot(vec)>0)){continue;};
loc0=calcPath.projection(vec);
pathElement.pathSegList.appendItem(pathElement.createSVGPathSegMovetoAbs(loc0.x,loc0.y));
xl=loc0.x;
yl=loc0.y;
...
pathElement.pathSegList.appendItem(pathElement.createSVGPathSegCurvetoQuadraticRel(Math.round(loc.x-xl),Math.round(loc.y-yl),Math.round(loc2.x-xl),Math.round(loc2.y-yl)));
//temp+="q"+Math.round(loc2.x-xl)+","+Math.round(loc2.y-yl)+","+Math.round(loc.x-xl)+","+Math.round(loc.y-yl);
} else {
pathElement.pathSegList.appendItem(pathElement.createSVGPathSegLinetoRel(Math.round(loc.x-xl),Math.round(loc.y-yl)));
...
calcPath=function(object,matrix,facingAway){
rdummy=1000/viewAngle;
var ro=rdummy/2;
var loc={x:0,y:0};
var loc0={x:0,y:0};
...
//rad=Math.atan2(Math.sqrt(vec.x*vec.x+vec.y*vec.y),vec.z)*rdummy;
//return {x:Math.cos(theta)*rad,y:Math.sin(theta)*rad}
l=vec.x*vec.x+vec.y*vec.y;
r=Math.atan2(Math.sqrt(l),vec.z)*rdummy;
...
coor=shapeList[i].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","IndexedLineSet")[0].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","Coordinate")[0].getAttributeNode("point").value.split(/ *, */,32000);
ind=shapeList[i].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","IndexedLineSet")[0].getAttributeNode("coordIndex").value.split("-1",32000);
noFaces=true;
};
if (shapeList[i].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","PointSet").length){
coor=shapeList[i].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","PointSet")[0].getElementsByTagNameNS("http://www.web3d.org/specifications/x3d-3.0","Coordinate")[0].getAttributeNode("point").value.split(/ *, */,32000);
noFaces=true;
points=true;
};
...
for (var i=0,j=0;painting.length>i;++i,++j){
if (!o[painting[i].index].faces){
//calcPathSegList(shapes[j],o[painting[i].index],painting[i].transform);
shapes[j].setAttribute("d",calcPath(o[painting[i].index],painting[i].transform));
...
shapes[j].setAttribute("d",calcPath(o[painting[i].index],painting[i].transform,false ));
...
shapes[j].setAttribute("d",calcPath(o[painting[i].index],painting[i].transform,true));
shapes[j].setAttribute("fill","none"); 
shapes[j].removeAttribute("fill-opacity")
shapes[j].setAttribute("stroke","#000000"); 
//calcPathSegList(shapes[++j],o[painting[i].index],painting[i].transform,false);
shapes[++j].setAttribute("d",calcPath(o[painting[i].index],painting[i].transform,false ));
shapes[j].setAttribute("fill-opacity",o[painting[i].index].transparency); 
shapes[j].setAttribute("fill",o[painting[i].index].colour); 
shapes[j].setAttribute("stroke","#000000"); 
}}
};
...
rateDisplay.textContent=parseInt(1000/(new Date()-drawAll.startTime));
};
drawAll.count=10;
//      for (i=0;1000>i;++i){
//svg.forceRedraw();
